...
export function SessionDialog({ open, semesterId, className, onClose, onSaved }: { open: boolean; semesterId: string; className: string; onClose: () => void; onSaved: (session: SessionSummary) => void }) {
  ...
  useEffect(() => {
    if (!open || !semesterId || !className || !date) {
    ...
    void (async () => {
      try {
        const params = new URLSearchParams({ semesterId, className, date });
  ...
  }, [open, semesterId, className, date]);
  ...
  async function submit(event: React.FormEvent) {
    ...
    setError("");
    try {
      const session = await requestJson<SessionSummary>(`/api/semesters/${semesterId}/session`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ className, date }) });
      onSaved(session);
      onClose();
    } catch (reason) {
    ...
    <Dialog open={open} title="新建课次" onClose={onClose}>
      <form onSubmit={submit} className="dialog-form">
        ...
        <div className="dialog-form__actions">
          ...
          <Button type="submit" disabled={saving || !semesterId || !className}>
            {saving ? "创建中…" : "创建课次"}
          </Button>
        </div>
      </form>
