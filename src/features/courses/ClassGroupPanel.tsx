...
interface ClassGroup { id: string; name: string; memberships: Array<{ classId: string; class: SemesterClass }>; lessons: GroupLesson[]; }
...
export function ClassGroupPanel({ semesterId, classes, sessions }: { semesterId: string; classes: SemesterClass[]; sessions: SemesterSession[] }) {
  ...
  function openGroup(group?: ClassGroup) {
    setEditingGroup(group ?? null); setGroupName(group?.name ?? ""); setGroupClassIds(group?.memberships.map((item) => item.classId) ?? []); setError(""); setGroupDialog(true);
  }
  async function saveGroup(event: React.FormEvent) {
    event.preventDefault(); setSaving(true); setError("");
    try {
      await requestJson(editingGroup ? `/api/class-groups/${encodeURIComponent(editingGroup.id)}` : `/api/semesters/${encodeURIComponent(semesterId)}/class-groups`, { method: editingGroup ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: groupName, classIds: groupClassIds }) });
  ...
  return <>
    <Section title="平行班与共同课" description="共同材料只在教师确认后形成修订；每个班的真实课次、评价和考勤仍保持独立。">
      ...
      {loading ? <p>正在读取共同课…</p> : groups.length === 0 ? <EmptyState title="暂无班级组" description="将同一学期的平行班放入一个组，随后建立共同课。" /> : <div className="class-group-list">{groups.map((group) => <article className="class-group-card" key={group.id}>
        <header><div><strong>{group.name}</strong><p>{group.memberships.map((item) => item.class.name ?? item.class.code).join("、")}</p></div><div><Button variant="ghost" uiSize="sm" onClick={() => openGroup(group)}>编辑组</Button><Button uiSize="sm" onClick={() => openLesson(group)}>新建共同课</Button></div></header>
