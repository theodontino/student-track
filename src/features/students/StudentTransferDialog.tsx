...
export function StudentTransferDialog({ workspace }: { workspace: Workspace }) {
  ...
  const classOptions = workspace.classOptions;
  ...
  return (
    ...
    >
      {student && <form onSubmit={(event) => { event.preventDefault(); void workspace.submitTransfer(); }} className="student-transfer-dialog">
        ...
        {classOptions.phase === "loading" && <StatusBanner tone="info">正在加载当前学期的班级列表…</StatusBanner>}
        {classOptions.phase === "error" ? (
          <StatusBanner tone="danger">
            班级列表加载失败：{classOptions.error}
            {classOptions.diagnosticId ? `（诊断编号：${classOptions.diagnosticId}）` : ""}
            <Button type="button" variant="secondary" onClick={classOptions.retry}>重试</Button>
          </StatusBanner>
        ) : (
          <FormField id="student-transfer-class" label="目标班级" description="只能选择当前学期已有班级，不允许手填班级名称。" required>
            <Select
              ...
              disabled={classOptions.phase !== "ready"}
            >
              <option value="">{classOptions.phase === "loading" ? "正在加载班级…" : "请选择目标班级"}</option>
              {targetClasses.map((klass) => <option key={klass.id} value={klass.id}>{klass.code}{klass.name ? ` · ${klass.name}` : ""}</option>)}
            </Select>
          </FormField>
        )}
        {classOptions.phase === "ready" && targetClasses.length === 0 && <StatusBanner tone="warning">当前学期没有可选的其他班级，请先建立目标班级。</StatusBanner>}
        ...
        <div className="student-transfer-dialog__actions">
          ...
          <Button type="submit" disabled={workspace.transferring || classOptions.phase !== "ready" || targetClasses.length === 0 || !workspace.transferClassId}>{workspace.transferring ? "转班中…" : "确认转班"}</Button>
        </div>
      </form>}
    </Dialog>
  );
