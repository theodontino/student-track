...
export default function FeedbackToolsWorkspace({ tool }: { tool: string }) {
  ...
  return <main className="feedback-workspace">
    ...
    {currentTool === "manual-facts" && <><Section title="人工课堂事实" description="人工录入和复核直接写入当前真实课次；助教表、STEP 和 ZIP 仍回到统一投料入口。"><div className="feedback-tool-actions">{capabilities.audioTranscription && <Link className="ui-button ui-button--secondary ui-button--md" href={`/diarize?semesterId=${encodeURIComponent(context.context.semesterId)}&class=${encodeURIComponent(context.context.className)}&sessionCode=${encodeURIComponent(context.context.sessionCode)}`}>录音转写</Link>}<Link className="ui-button ui-button--ghost ui-button--md" href="/feedback">助教表 / STEP / ZIP 统一投料</Link></div></Section><EntryWorkspace /></>}
    ...
    {currentTool === "materials" && <Section title="公共材料" description="公共材料只作为课程背景，确认修订后复制进任务快照。">{context.data?.groupProgress?.lesson?.confirmedMaterial ? <pre className="feedback-tool-material">{context.data.groupProgress.lesson.confirmedMaterial.groupFeedbackRaw}</pre> : context.data?.sessionCommonMaterial?.material ? <pre className="feedback-tool-material">{context.data.sessionCommonMaterial.material.groupFeedbackRaw}</pre> : <p>当前课次没有已确认公共材料。</p>}<Link className="ui-button ui-button--secondary ui-button--md" href={`/semesters/${encodeURIComponent(context.context.semesterId)}#semester-common-materials`}>管理学期公共材料</Link></Section>}
