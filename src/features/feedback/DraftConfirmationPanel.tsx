...
export function DraftConfirmationPanel({ workspace }: { workspace: Workspace }) {
  return (
    <Section title="结构化记录确认" description="确认后才会写入评价、考勤、事件和沟通。已写入后仍可补充教师观察并更新。" actions={<Button onClick={() => void workspace.confirm()} disabled={!workspace.canConfirm}>{workspace.confirming ? "写入中…" : workspace.confirmed ? "更新写入" : "确认写入"}</Button>}>
      <div className="feedback-draft-confirmation">
        ...
        {!workspace.parsedResult ? <EmptyState title="等待解析课堂回顾" description="解析后，学生记录会显示在这里供你确认。" /> : <div className="feedback-draft-list">{workspace.parsedResult.students.map((student, index) => <DraftStudentCard key={`${student.name}-${index}`} student={student} onAttendanceChange={(present) => workspace.setParsedAttendance(index, present)} onInterventionsChange={(interventions) => workspace.setParsedTeacherInterventions(index, interventions)} />)}</div>}
      </div>
    </Section>
  );
}
