...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await context.params;
    const body = await request.json().catch(() => ({})) as { action?: unknown; plan?: unknown; decisions?: unknown };
    const action = body.action === "apply" || body.action === "confirm" || body.action === "resolve" || body.action === "create_plan" ? body.action : null;
    if (!action) return NextResponse.json({ error: "action 必须是 confirm、create_plan 或兼容的 apply" }, { status: 400 });
    const decisions = Array.isArray(body.decisions) ? body.decisions as FeedbackIntakeDecision[] : [];
    const result = await resolveFeedbackIntakeRun(id, { action, decisions, plan: body.plan as never });
