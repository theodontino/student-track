...
import { FeedbackPlanBatchActionSchema } from "@/lib/feedback-plan-batch";
...
export async function POST(request: NextRequest, context: Context) {
  try {
    ...
    if (input.action === "start") return NextResponse.json(await startFeedbackPlanBatch(id), { status: 202 });
