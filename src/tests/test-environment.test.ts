...
describe("isolated test environment safety", () => {
  it("accepts only the dedicated test database under a prefixed system temp directory", () => {
    ...
    const url = sqliteFileUrl(path.join(directory, "test.db"));
  ...
  it("rejects the project database and unrelated temp paths", () => {
    expect(() => assertSafeTestDatabaseUrl(sqliteFileUrl(path.join(process.cwd(), "dev.db"))))
      .toThrow("unsafe test directory");
    expect(() => assertSafeTestDatabaseUrl(sqliteFileUrl(path.join(os.tmpdir(), "unrelated", "test.db"))))
      .toThrow("unsafe test directory");
