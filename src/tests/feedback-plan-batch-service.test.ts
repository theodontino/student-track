...
describe("feedback plan batch service", () => {
  ...
  it("rejects every historical batch generation control without modifying its rows", async () => {
    const batch = await createFeedbackPlanBatch({ requestKey: `${marker}-RETIRED`, semesterId, type: "event_micro", outputRequirement: "只读历史", plans: classIds.map((classId, index) => ({ classId, sessionId: sessionIds[index], studentIds: [studentIds[index]] })) });
    const before = await prisma.feedbackPlanBatch.findUniqueOrThrow({ where: { id: batch.id }, include: { plans: { include: { items: true } } } });
    for (const action of [startFeedbackPlanBatch, pauseFeedbackPlanBatch, continueFeedbackPlanBatch, forceStopFeedbackPlanBatch, retryFeedbackPlanBatch, retryFeedbackPlanBatchWithFree]) {
      await expect(action(batch.id)).rejects.toMatchObject({ status: 409, code: "legacy_generation_retired" });
    }
    expect(await prisma.feedbackPlanBatch.findUniqueOrThrow({ where: { id: batch.id }, include: { plans: { include: { items: true } } } })).toEqual(before);
  });
