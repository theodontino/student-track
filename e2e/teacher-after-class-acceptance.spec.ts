...
test.describe("教师纯页面课后验收", () => {
  ...
  test("教师只操作页面完成单班课后反馈", async ({ page, request }) => {
    ...
    await test.step("准备任务：投料、选人、公共材料和策略", async () => {
    ...
    await test.step("核对并确认：一次处理来源异常，事实和范围分开确认", async () => {
      ...
      await page.reload();
      await expect(page.getByText("2 个来源 · 事实已确认")).toBeVisible();
      await page.getByRole("button", { name: "进入核对并确认" }).click();
      await expect(page.getByText("材料与事实已确认")).toBeVisible();
      await page.getByRole("button", { name: "确认班级、课次和反馈对象" }).click();
      await expect(page.getByText("班级、课次和反馈对象已确认")).toBeVisible();
      await expect(page.getByRole("button", { name: "创建并开始快速草稿" })).toBeVisible();
    });

    await test.step("生成与恢复：从最终按钮进入工作室并在刷新后继续", async () => {
      await page.getByRole("button", { name: "创建并开始快速草稿" }).click();
