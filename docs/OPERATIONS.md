...
默认课后工作台只创建单班反馈计划。班级组继续提供共同进度和公共材料，但各班分别进入反馈流程。1.2 已存在的多班批次仍可从当前任务或历史深链接打开、恢复、导出和归档；系统不再提供新建批次入口。
...
这条 WebKit 用例不会用测试代码直接调用本仓 API 完成业务动作。教师的投料、异常选择、事实确认、
范围确认、生成、刷新恢复、正文保存、附件、批准、Excel/WCG 草稿下载、归档和历史查看全部通过页面
...
扫描只整理候选事实，不写课堂事实。教师先按来源处理日期/课次边界、身份和确定性冲突，再以一个事务确认材料与事实；随后单独确认班级、课次和反馈对象。只有全部确认后，Task API 才先创建 Plan/Batch、返回 ID，再启动生成；启动失败仍进入工作室重试。默认第三阶段使用统一计划工作室，左侧学生列表独立滚动，右侧处理证据、重试、独立设置、正文保存、批准与导出。刷新以 `intakeRunId`、`planId` 或 `batchId` 从服务端恢复；高级操作从 `/feedback/tools` 进入，不再存在第二套五步状态。

活动任务工具显示全部未归档 Plan/Batch。归档单班任务会解除该计划对新任务的阻断；归档批次会在事务中同时归档子计划。若页面提示活动任务冲突，应从工具中打开冲突任务或明确归档后重建，不需要直接修改数据库。

## 班级组共同进度（1.2 Beta 4）

新建或编辑班级组时必须指定成员班中的主班。主班新建课次会进入最早尚无主班课次的共同课；没有待用共同课时自动建立下一讲。其他班新建课次会进入主班已开始、该班尚未完成的最早一讲；主班尚未推进时，本课保持独立并显示提醒。教师可在学期详情调整课次对应的共同课或设为独立，调整不移动评分、考勤或事件。

反馈页选择组内课次后会显示班级组、主班、讲次和材料确认状态。整理课程材料时会写入当前共同课草稿；本班当前 FeedbackPlan 可使用本次材料快照，同组其他班只有在教师点击确认修订后才读取。既有班级组迁移后不会自动猜测主班，需在管理页补选后才启用自动进度。

## 学期公共材料与主反馈（1.2）

学期公共材料库（原话术库）按课次保存结构化材料，旧库读取时兼容转换。主班创建第 N 讲共同课时，系统只把话术库第 N 课复制为草稿；教师可以改选其他课次或在反馈入口明确跳过，确认后才形成共享修订。非主班沿用已开始的同一 `GroupLesson`，不重新录入材料。独立课次可在三段式反馈第一阶段选择一条话术并明确保存为本课快照。
