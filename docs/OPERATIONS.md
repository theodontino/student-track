...
企微提取模型在“系统中心 → LLM 配置 → 模型角色分工”单独指定。兼容性顺序固定为 `json_schema + reasoning_effort:none`、保留 Schema 并去掉不兼容的推理参数、再尝试 `json_object`；不允许降级到普通文本。自动提取使用 `wecom-grounded-v5-feedback-triage`：模型必须返回消息 ID、事实摘要、反馈用途分类、优先级和 1–3 条原文短句，短句逐字核验失败即进入复核且不自动重试。网络错误只重试一次；连续 3 个证据失败，或前 20 批累计 5 个证据失败时暂停剩余批次。
...
手动流程仍可生成候选 JSON 后预览确认。一键流程只自动写入唯一学生、高置信度、原文证据逐字匹配、课次可确定且对课后反馈具有中高价值的摘要；收悉、感谢、排课、报名缴费、接送、普通请假、文件发送、群通知和无新增事实的寒暄直接标记为无有效内容。沟通对象统一写“家长”，实际沟通时间和反馈用途随摘要保留，企微自动导入不创建学生关注标签。任何越界、低置信度、证据或课次问题都会转入待复核。
