...
1.1 的语气（温和 / 专业）和详略（简洁 / 详细）属于当前输入的一部分。界面仍显示
去除空白、保留标点后的可见字符数，但它只供教师参考，不改变审核状态或阻止导出。
单人重试可以重新选择这两个维度；换模型生成只使用页面选择的安全配置摘要，不改变
系统激活配置。模型、语气或详略变化会产生独立版本，新版本需显式“采用此版本”后
才影响导出。

批量生成在取消、断网或页面刷新后会恢复为“批次未完成”，并显示已保留数量。可以只继续未完成/失败学生、把当前部分结果显式保存到工作历史，或放弃旧批次。部分结果和输入已经变化的旧结果都不能导出；继续前会重新核对课程材料、PDF 证据、反馈档位和输出策略的输入指纹。该机制只覆盖当前标签页和教师主动保存的工作历史，不承诺浏览器会话消失或服务重启后的后台任务续跑。
...
“工作历史 → AI 生成”提供教学记忆的人工维护入口。选择班级后，“生成到期长期背景草案”只检查来源课次已满六个月的温层记录，并从对应的已确认学期快照读取受控摘要；没有可靠摘要时会安全跳过，不调用模型。生成后的草案必须由教师逐条核对和确认，确认前不会清理温层细节。
...
课后工作台的反馈计划支持班级公共、事件微反馈、阶段趋势和结课教学总结四类类型。教师先创建计划、检查证据篮子和可选模块，再批准条目；只有已批准文本可导出。默认完整导出会在存在未批准条目时阻断；需要先处理阻断项，或显式选择“仅导出已批准项”。部分导出会写入 `FeedbackExportRun`，后续补导时以条目 ID 和最终文本哈希提示已导出内容。
