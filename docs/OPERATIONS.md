...
Invoke-WebRequest -UseBasicParsing -Uri "https://github.com/theodontino/student-track/releases/download/v1.3.1/Install-StudentTrackCore.ps1" -OutFile $installer
...
“系统中心 → 维护与操作日志”可下载 JSON 诊断包，教师下载后手动发送给维护人员。文件只在请求内存中生成，
不会保存导出副本，也不会读取或写入 `SystemLog`。它只包含产品版本、edition、操作系统和 Node 运行时摘要、
系统能力、数据库可读状态、迁移状态、数据根目录、诊断账本、反馈附件、反馈收件箱与备份目录的存在/访问状态，
以及最近七天最多 500 条白名单故障事件。
诊断包不包含学生或业务 ID、正文、文件名或路径、目录大小、业务计数、环境变量值、凭据、原始异常、堆栈或
原始系统日志。故障账本只记录诊断 ID、时间、固定功能范围、HTTP 5xx 状态、受控错误码和是否可重试；账本写入失败不会影响业务结果。
