...
课后工作台的反馈计划支持班级公共、事件微反馈、阶段趋势和结课教学总结四类类型。教师先创建计划、检查证据篮子和可选模块，再批准条目；只有已批准文本可导出。已确认的个人出门测证据会随创建或重新生成请求进入对应学生的证据篮子，提炼结果保存在计划证据快照中，重新打开计划或未重新上传 PDF 时仍可继续使用；原始 PDF 不落盘。重新生成只处理教师当前勾选且仍可修改的条目。证据摘要分别显示“初稿覆盖/必覆盖”和“成稿保留/必覆盖”数量，并可展开查看模型全证据初稿；初稿缺少任何教学或测评证据会自动纠错，仍缺失则生成失败。教师编辑最终成稿时可以主动删减，系统会把删减项显示为待教师确认而不是阻断或伪报全覆盖。默认完整导出会在存在未批准条目时阻断；需要先处理阻断项，或显式选择“仅导出已批准项”。部分导出会写入 `FeedbackExportRun`，后续补导时以条目 ID 和最终文本哈希提示已导出内容。
