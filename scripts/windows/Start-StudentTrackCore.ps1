...
$preparationScript = if ($OfflineBundle) {
    "Prepare-StudentTrackCoreOffline.ps1"
} else {
    "Prepare-StudentTrackCore.ps1"
}

if (-not (Test-Path -LiteralPath $runtime.DatabasePath -PathType Leaf)) {
    throw "尚未初始化数据库，请先运行 scripts\windows\$preparationScript。"
}
if (-not (Test-Path -LiteralPath $buildId -PathType Leaf)) {
    throw "尚未生成生产构建，请先运行 scripts\windows\$preparationScript。"
}
if (-not (Test-Path -LiteralPath $requiredServerFiles -PathType Leaf)) {
    throw "生产构建缺少版本信息，请重新运行 scripts\windows\$preparationScript。"
}
...
if ($buildEdition -ne "core") {
    throw "当前生产构建不是 Student Track Core，请重新运行 scripts\windows\$preparationScript。"
}
