...
当前开发版本为 **1.3.0**（Zhuiver `MINOR`，正式发布「反馈引擎与多平台分发更新」）。同一套源码和数据库按构建配置提供 Full 与 Core：macOS 默认保持 Full，Windows 10/11 x64 首先支持 Core。课后工作台保留“录入—规划—生成”主流程；学生受限反馈由 Planner 只整理冻结事实、Writer 只读取 ContentBrief，已有候选草稿始终保留供教师复核。一个反馈计划可组织多个班级的学生，生成会按服务状态自适应调节并发；A/B/C/D 含义在录入页直接展示。
...
| 课后工作台 | 统一事实录入、命名计划、队列生成、全过程回看、安全修订、教师编辑批准和 Excel 导出 |
...
- 课后工作台保持“录入 → 规划 → 生成”三段；三段可随时回看并以 URL 恢复。录入确认后的事实独立保存，规划草稿以命名 FeedbackPlan/Batch 自动保存，生成开始后只能通过新计划修订。
- `/feedback/tools` 提供人工事实、PDF、公共材料、特殊计划、手工批次和活动任务管理，但不再维护第二套反馈步骤或恢复状态。
