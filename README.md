...
当前开发版本为 **1.3.0-beta.6**（Zhuiver `PATCH` 预发布）。同一套源码和数据库按构建配置提供 Full 与 Core：macOS 默认保持 Full，Windows 10/11 x64 首先支持 Core。课后工作台保留“录入—规划—生成”主流程；学生受限反馈恢复为 Planner 只整理冻结事实、Writer 只读取 ContentBrief 的链路，已有候选草稿始终保留供教师复核。A/B/C/D 含义在录入页直接展示。
