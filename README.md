...
当前开发版本为 **1.3.1**（Zhuiver `PATCH`，正式发布「表格工作流与维护性更新」）。机构原始花名册可以直接分析并按班筛选，反馈 Excel 可直接用于教师精灵，真实课次可以下载并回传助教评分模板；反馈批量导出、Windows 转班恢复和隐私安全诊断也得到补强。Full 与 Core 继续共用同一业务数据和 Prisma Schema，课后工作台仍保持“录入—规划—生成”主流程。

## 实验性能力：STEP Bridge

Student Track 当前包含实验性的 STEP 人工文件桥，用于验证 ST 与 STEP 之间的班级花名册和课堂事实流转。该能力**不属于当前稳定产品承诺，也不作为当前 Student Track 的正式跨仓协议交付要求**。

1.3.1 继续保留人工文件桥；附近设备直连不在本次发布范围，顺延到 1.3.2 继续实验。
