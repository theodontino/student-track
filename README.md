...
当前正式版本为 **1.2.5**（Zhuiver `PATCH`）。反馈入口已收敛为唯一三段式流程；单班和班级组共享同一套材料确认、任务创建、生成复核与恢复规则，高级能力集中在同一数据权威下的工具套装。
...
- 课后工作台只有“准备任务 → 核对并确认 → 生成与复核”三段；未提交选择保存在当前标签页，事实确认、范围确认、计划与批次状态以服务端账本为准。
