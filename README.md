...
当前开发版本为 **1.3.0-beta.5**（Zhuiver `PATCH` 预发布）。同一套源码和数据库按构建配置提供 Full 与 Core：macOS 默认保持 Full，Windows 10/11 x64 首先支持 Core。课后工作台保留“录入—规划—生成”主流程，补齐 STEP V1/V2 与大批量文件夹导入；文字型出门测的正确率会换算为一位小数 A 分，和已有事实冲突时由教师逐项或按同一策略批量确认。A/B/C/D 含义在录入页直接展示。
