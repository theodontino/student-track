...
当前开发版本为 **1.3.0-beta.4**（Zhuiver `PATCH` 预发布）。同一套源码和数据库按构建配置提供 Full 与 Core：macOS 默认保持 Full，Windows 10/11 x64 首先支持 Core。课后工作台保留“录入—规划—生成”主流程，并用当前计划栏、切换 Drawer 与正文优先的逐学生工作区降低复核负担。新计划只使用受限或自由生成；历史 legacy 计划保留已有正文、批准和导出，但停止继续调用模型。
